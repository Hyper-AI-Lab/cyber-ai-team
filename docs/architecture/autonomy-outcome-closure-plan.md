...
- Use a bounded, schema-constrained action-selection contract for immutable options. The
  ...
  response is limited to disposition, the exact option reference, confidence, and a bounded
  reason code. The trusted context includes the current durable mandate authority, and server
  code expands the reason into rationale and unknowns in the durable mandate result. This
  keeps local FOSS inference usable under constrained compute without moving authority,
  evidence, impact, expected effects, or tool parameters into model-generated text.
