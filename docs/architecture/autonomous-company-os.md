...
- **FOSS-first resource policy:** new tools, modules, and services must be free and open-source or self-hosted using current infrastructure. Paid or SaaS-only resources are future options, not readiness requirements. An owner may explicitly authorize a metered inference provider as a narrow runtime exception; that exception does not permit proprietary tools, automatic subscription changes, or unbounded spend elsewhere in the system.
