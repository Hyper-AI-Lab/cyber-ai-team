...
async def test_internal_audit_acquisition_skips_feedback_and_routine_success(
    ...
    assert source.cursor["last_audit_id"] == "audit-6"


@pytest.mark.asyncio
async def test_internal_memory_requires_explicit_company_evidence_promotion(
    intelligence_session_factory,
):
    service = CompanyIntelligenceService()
    await service.ensure_default_sources("company:test")
        ...
        session.add_all(
            [
                MemoryEntry(
                    ...
                    namespace="company:test:knowledge",
                    content="Verified customer interview summary.",
                    metadata_={
                ...
                MemoryEntry(
                    id="memory-runtime-output",
                    agent_id=None,
                    memory_type="procedural",
                    namespace="company:test:executive",
                    content="Routine reflection generated by the autonomy loop.",
                    metadata_={"source_type": "executive_reflection"},
                    created_at=now + timedelta(seconds=1),
                ),
            ]
        )
    ...
    assert [item.external_id for item in signals] == ["memory-evidence"]
    assert source.cursor["last_memory_id"] == "memory-runtime-output"
...
async def test_untrusted_research_becomes_provenance_backed_capped_claims(
    ...
    assert llm.invoke_json.await_args.kwargs["max_tokens"] == 512
    assert llm.invoke_json.await_args.kwargs["json_schema"]["required"] == ["claims"]
    value_schema = llm.invoke_json.await_args.kwargs["json_schema"]["properties"][
        "claims"
    ]["items"]["properties"]["value"]
    assert value_schema["required"] == ["summary"]
