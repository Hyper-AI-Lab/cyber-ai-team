...
ALLOW_LIVE_SIDE_EFFECTS = os.environ.get(
    "COMPOSE_SMOKE_ALLOW_LIVE_SIDE_EFFECTS",
    "false",
).lower() in {"1", "true", "yes", "on"}
EMAIL_RECIPIENT = os.environ.get("COMPOSE_SMOKE_EMAIL_RECIPIENT", "smoke-test@example.com")
...
def main() -> int:
    ...
    if email_tool.get("state") == "live" and not ALLOW_LIVE_SIDE_EFFECTS:
        print("Rejecting approval instead of replaying live email side effect")
        status, rejection = request_json(
            "POST",
            f"{API_BASE}/api/dashboard/approval/{approval_id}/reject",
            {"note": "Rejected by Compose smoke test; live side effects are disabled."},
            token=access_token,
        )
        assert_true(status == 200, f"Approval rejection failed: {status} {rejection}")
        assert_true(
            rejection.get("status") == "rejected",
            f"Unexpected rejection response: {rejection}",
        )
        print("Compose smoke test passed.")
        return 0
    ...
    assert_true(
        any(item.get("recipient") == EMAIL_RECIPIENT for item in logs),
